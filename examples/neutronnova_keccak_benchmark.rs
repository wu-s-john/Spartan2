...
//! examples/neutronnova_keccak_benchmark.rs
//! Benchmark NeutronNova NIFS folding of Keccak-256 hash chains comparing:
//! - Small-value NIFS sumcheck
//! - Large-value (vanilla) NIFS sumcheck
//!
//! Run with: `RUST_LOG=info cargo run --release --example neutronnova_keccak_benchmark`
...
#[derive(Parser)]
#[command(about = "NeutronNova Keccak benchmark: small vs large sumcheck")]
struct Args {
    ...
    + DelayedReduction<E::Scalar>,
{
  ...
  let header = format!(
    "===== NeutronNova Keccak ZkProve: instances={}, chain_length={}, constraints={}, cores={} =====",
    num_instances, chain_length, pk.S_step.num_cons, num_cores,
  );
  print_table(
    &header,
