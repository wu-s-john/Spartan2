...
use spartan2::rs_shuffle::{
  ...
  gadget::{rs_shuffle, rs_shuffle_with_reencryption},
...
impl ConstraintSynthesizer<BaseField> for RsShuffleCircuit {
  fn generate_constraints(
    ...
    // Allocate seed
    let seed_var = FpVar::new_input(cs.clone(), || Ok(self.seed)).expect("Failed to allocate seed");
    ...
    // Allocate public key
    let pk_var = GrumpkinVar::new_variable(cs.clone(), || Ok(self.pk), AllocationMode::Input)
      .expect("Failed to allocate public key");

    // Allocate challenges
    let alpha_var =
      FpVar::new_input(cs.clone(), || Ok(self.alpha)).expect("Failed to allocate alpha");
    let beta_var = FpVar::new_input(cs.clone(), || Ok(self.beta)).expect("Failed to allocate beta");
...
fn main() {
  ...
  // Allocate seed
  let seed_var = FpVar::new_input(cs_full.clone(), || Ok(seed)).expect("Failed to allocate seed");

  // Allocate input ciphertexts
  let ct_input_vars: [ElGamalCiphertextVar<GrumpkinProjective, GrumpkinVar>; N] =
    std::array::from_fn(|i| {
      ElGamalCiphertextVar::new_variable(cs_full.clone(), || Ok(&ct_init[i]), AllocationMode::Input)
        .expect("Failed to allocate input ciphertext")
    });
...
fn run_spartan_proof(
  ...
  use spartan2::{
    arkworks::{ArkworksCircuitAdapter, convert_constraint_matrices, extract_assignments},
...
fn run_spartan_zk_proof(
  ...
  use spartan2::{
    arkworks::{
      ArkworksCircuitAdapter, convert_constraint_matrices, extract_assignments,
      extract_witness_only,
    },
    provider::Bn254Engine,
    spartan_zk::SpartanZkSNARK,
    timing::{SPARTAN_ZK_PHASES, TimingLayer, clear_timings, snapshot_timings},
    traits::snark::R1CSSNARKTrait,
  };
  use std::time::Instant;
  use tracing_subscriber::prelude::*;
  ...
  let rerandomizations_setup: [BaseField; N] = std::array::from_fn(|_| BaseField::rand(&mut rng));
  ...
  let rerandomizations_prove: [BaseField; N] = std::array::from_fn(|_| BaseField::rand(&mut rng));
  ...
  let (w_prove, x_prove) = extract_witness_only::<BaseField, Bn256Fr, _>(circuit_prove)
