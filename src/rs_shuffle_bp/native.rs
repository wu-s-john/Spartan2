...
use super::data_structures::{PermutationWitnessTrace, SortedRow, UnsortedRow};
use ff::PrimeField;
...
/// Derive split bits from a seed using simple hash-based generation
///
/// Uses the seed's byte representation to deterministically generate bits.
pub fn derive_split_bits<F: PrimeField, const N: usize, const LEVELS: usize>(
  seed: F,
) -> ([[bool; N]; LEVELS], usize) {
  use sha2::{Digest, Sha256};

  let seed_bytes = seed.to_repr();
  let seed_slice = seed_bytes.as_ref();

  let total_bits_needed = N * LEVELS;
  let bytes_needed = (total_bits_needed + 7) / 8;

  // Generate enough random bytes using SHA256 chain
  let mut all_bytes = Vec::with_capacity(bytes_needed);
  let mut current_hash = seed_slice.to_vec();

  while all_bytes.len() < bytes_needed {
    let mut hasher = Sha256::new();
    hasher.update(&current_hash);
    let result = hasher.finalize();
    all_bytes.extend_from_slice(&result);
    current_hash = result.to_vec();
  }

  // Convert bytes to bits
  let mut bit_stream: Vec<bool> = Vec::with_capacity(total_bits_needed);
  for byte in all_bytes.iter().take(bytes_needed) {
    for bit_idx in 0..8 {
      if bit_stream.len() < total_bits_needed {
        bit_stream.push((byte >> bit_idx) & 1 == 1);
      }
    }
  }

  // Fill the N×LEVELS matrix
  let bit_matrix: [[bool; N]; LEVELS] = std::array::from_fn(|level| {
    std::array::from_fn(|i| {
  ...
  // Return number of SHA256 calls as "samples"
  let num_samples = (bytes_needed + 31) / 32;
...
mod tests {
  use super::*;
  use halo2curves::pasta::Fp as PallasScalar;
  ...
  fn test_run_rs_shuffle_permutation() {
    let seed = PallasScalar::from(42u64);
    let input: [usize; N] = std::array::from_fn(|i| i);

    let trace = run_rs_shuffle_permutation::<PallasScalar, _, N, LEVELS>(seed, &input);
