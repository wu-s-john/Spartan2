  ...
  E::Scalar: SmallValueEngine<SV>,
{
  ...
  // - e_in (inner loop): expanded_eq_left() from FIRST first_half taus, size 2^first_half
  // - e_xout (outer loop): eq_evals_right() from LAST second_half taus, size 2^second_half
  let e_in = eq_instance.expanded_eq_left();
  ...
  let accumulators =
    build_accumulators_spartan(poly_A_small, poly_B_small, &taus, l0, &e_in, e_xout);
...
mod tests {
    ...
    F: SmallValueEngine<SV>,
  {
    ...
    // - e_in (inner loop): FIRST first_half taus, size 2^first_half
    ...
    let e_in = suffix_eq_instance.expanded_eq_left();
    let e_xout = suffix_eq_instance.eq_evals_right();

    let accs =
      build_accumulators_spartan(&az_poly, &bz_poly, &taus, SMALL_VALUE_ROUNDS, &e_in, e_xout);
