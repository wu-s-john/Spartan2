...
use super::{
  bit_generation::derive_split_bits_gadget,
  data_structures::{
    ElGamalCiphertextVar, PermutationWitnessTraceVar, SortedRowVar, UnsortedRowVar,
  },
  encryption::ElGamalEncryption,
  permutation::{IndexPositionPair, IndexedElGamalCiphertext, check_grand_product},
};
  ...
  CV: CurveVar<C, C::BaseField>,
{
  ...
  check_grand_product::<C::BaseField, IndexedElGamalCiphertext<C, CV>, 5>(
    ...
    &ciphertexts_initial,
    &ciphertexts_final,
    &[alpha.clone(), beta.clone(), beta_2, beta_3, beta_4],
  )?;
  ...
  CV: CurveVar<C, C::BaseField>,
{
  ...
  // ═══════════════════════════════════════════════════════════════
  let bits_mat =
    derive_split_bits_gadget::<C::BaseField, N, LEVELS>(cs.clone(), seed, num_samples)?;
...
mod tests {
  use super::*;
  use crate::rs_shuffle::{data_structures::ElGamalCiphertext, native::run_rs_shuffle_permutation};
  ...
  use ark_r1cs_std::{
    alloc::AllocVar, groups::curves::short_weierstrass::ProjectiveVar, prelude::AllocationMode,
  };
  ...
  fn test_rs_shuffle_circuit() {
    ...
    let ct_init_vars: [ElGamalCiphertextVar<GrumpkinProjective, GrumpkinVar>; N] =
      std::array::from_fn(|i| {
        ElGamalCiphertextVar::new_variable(cs.clone(), || Ok(&ct_init[i]), AllocationMode::Witness)
    ...
    let beta =
      FpVar::new_input(cs.clone(), || Ok(BaseField::from(23u64))).expect("Failed to allocate beta");
