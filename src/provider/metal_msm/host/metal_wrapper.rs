use crate::provider::metal_msm::{
  host::gpu::{create_buffer, create_empty_buffer, create_uninitialized_buffer, get_default_device, read_buffer},
  utils::{
    barrett_params::calc_barrett_mu,
    mont_params::{calc_mont_radix, calc_nsafe, calc_rinv_and_n0},
  },
};
...
impl MetalHelper {
  ...
  pub fn create_uninitialized_buffer(&mut self, size: usize) -> Buffer {
    let buffer = create_uninitialized_buffer(&self.device, size);
    self.buffers.push(buffer.clone());
    buffer
  }
  ...
  pub fn execute_shader_with_pipeline(
    ...
    self.encode_shader_with_pipeline(
      command_buffer,
      pipeline_state,
      buffers,
      thread_group_count,
  ...
  pub fn encode_shader_with_pipeline(
    &self,
...
fn pallas_modulus() -> BigUint {
  BigUint::from_str("28948022309329048855892746252171976963363056481941560715954676764349967630337")
    .unwrap()
}
