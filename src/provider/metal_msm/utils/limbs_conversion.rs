...
/// Pack affine points into GPU-ready u32 buffers.
///
/// Each point contributes 8 (x) + 8 (y) = 16 packed u32s.
pub fn pack_affine_points<C: CurveAffine>(bases: &[C], _num_limbs: usize) -> Vec<u32>
where
  ...
  bases
    ...
    .for_each(|(base_chunk, coord_chunk)| {
      for (i, pt) in base_chunk.iter().enumerate() {
        ...
        coord_chunk[coord_start..coord_start + packed_per_coord].copy_from_slice(&x_packed);
...
/// Pack scalars into GPU-ready u32 buffers.
///
/// Each scalar contributes 8 packed u32s.
pub fn pack_scalars<F: PrimeField>(scalars: &[F], _num_limbs: usize) -> Vec<u32> {
  let packed_per_scalar = 8; // 16 limbs / 2
  let mut scalars_u32 = vec![0u32; scalars.len() * packed_per_scalar];

  const CHUNK_SIZE: usize = 1024;

  scalars
    .par_chunks(CHUNK_SIZE)
    .zip(scalars_u32.par_chunks_mut(CHUNK_SIZE * packed_per_scalar))
    .for_each(|(scalar_chunk, scalar_u32_chunk)| {
      for (i, sc) in scalar_chunk.iter().enumerate() {
        ...
        let scalar_start = i * packed_per_scalar;
        scalar_u32_chunk[scalar_start..scalar_start + packed_per_scalar]
...
{
  let coords = pack_affine_points(bases, num_limbs);
  let scalars_u32 = pack_scalars(scalars, num_limbs);
