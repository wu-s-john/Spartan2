...
#define INIT_LIMB(i) CURVE_BASEFIELD_MODULUS[i]
...
#pragma unroll(16)
    for (uint i = 0; i < NUM_LIMBS; i++) {
        p.limbs[i] = CURVE_BASEFIELD_MODULUS[i];
    }
    return p;
}
...
Jacobian get_curve_zero()
{
    Jacobian zero;

#pragma unroll(16)
    for (uint i = 0; i < NUM_LIMBS; i++) {
        zero.x.limbs[i] = CURVE_ZERO_X[i];
        zero.y.limbs[i] = CURVE_ZERO_Y[i];
        zero.z.limbs[i] = CURVE_ZERO_Z[i];
    }
    return zero;
}

Jacobian get_curve_one()
{
    Jacobian one;

#pragma unroll(16)
    for (uint i = 0; i < NUM_LIMBS; i++) {
        one.x.limbs[i] = CURVE_ONE_X[i];
        one.y.limbs[i] = CURVE_ONE_Y[i];
        one.z.limbs[i] = CURVE_ONE_Z[i];
    }
    return one;
}

Jacobian get_curve_zero_mont()
{
    Jacobian zero;

#pragma unroll(16)
    for (uint i = 0; i < NUM_LIMBS; i++) {
        zero.x.limbs[i] = CURVE_ZERO_XR[i];
        zero.y.limbs[i] = CURVE_ZERO_YR[i];
        zero.z.limbs[i] = CURVE_ZERO_ZR[i];
    }
    return zero;
}

Jacobian get_curve_one_mont()
{
    Jacobian one;

#pragma unroll(16)
    for (uint i = 0; i < NUM_LIMBS; i++) {
        one.x.limbs[i] = CURVE_ONE_XR[i];
        one.y.limbs[i] = CURVE_ONE_YR[i];
        one.z.limbs[i] = CURVE_ONE_ZR[i];
    }
    return one;
}
