...
/// Print a single-row table of timing data (no comparison).
pub fn print_single_table(header: &str, phases: &[(&str, &str)], timings: &HashMap<&str, u64>) {
  let col_w = 12;
  ...
  eprint!("{:<6}", "ms");
  for (_, short_name) in phases {
    let v = timings.get(short_name).copied().unwrap_or(0);
    eprint!("{:>width$}", v, width = col_w);
  }
  eprintln!();
}
